// Demo file with Stage 1 and Stage 2 fixes applied.
...
static void print_banner(void) {
    printf("=== Demo Program (Stage 2 pass) ===\n");
}
...
// Stage 2 fixed: coupon only applies before payment
static int apply_coupon_before_checkout(int paid, int coupon_applied) {
    int total = 100;
    if (!paid && coupon_applied) {
        total = total - 10;
    }
    if (total < 0) {
        total = 0;
...
// Stage 2 fixed: enforce authorization check
...
int main(int argc, char **argv) {
    ...
    (void)apply_coupon_before_checkout(1, 1);
    view_admin_report(1);
    log_info("Demo completed without known vulnerabilities.");
    print_footer();
    return 0;
}
